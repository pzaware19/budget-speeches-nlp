...
To measure this I scraped the full Economic Surveys (all chapters, English) for 2019 to 2025 from indiabudget.gov.in, between 87,000 and 332,000 words each, and computed the TF-IDF cosine similarity between every Survey and every budget speech. The same-year cell, a Survey and the budget it preceded by a day, is outlined in red. The off-diagonal cells are the baseline: how similar a Survey is to *other* years' budgets.

![TF-IDF cosine similarity between each full Economic Survey (row) and each budget speech (column). Darker is more shared distinctive vocabulary. Same-year cells, where the Survey was tabled the day before that budget, are outlined in red. The diagonal is the brightest cell in its row in only two of seven years.](output/figures/fig_es_bs_similarity.png){width=100%}

Two results, and the second is the interesting one. First, the Survey and the budget are broadly similar documents: on overall word usage their cosine similarity is about 0.48, which is high and unsurprising given they share a ministry, an economy, and a moment. Second, the budget does **not** specifically echo its own Survey more than it echoes neighbouring years' Surveys. The same-year similarity on distinctive vocabulary (0.187) is barely above the other-year average (0.166), a lift of only 1.13 times. In the heatmap the same-year cell is the brightest in its row in just two of seven years, 2020 and 2021. For 2022 through 2025 the Survey most resembles some *other* year's budget, and the recent Surveys cluster most tightly around the 2022-23 budget, a speech that happened to use unusually broad economic vocabulary.

The interpretation is the payoff. You might expect the budget, delivered the morning after the Survey, to read as a restatement of it. The text says it does not. The Finance Minister's speech is its own document, organised around the government's political priorities rather than the Chief Economic Adviser's diagnosis. The Survey and the budget are written in the same broad economic language, but the budget is not the Survey set to music.

::: {.callout-note}
This covers 2019 to 2025, which is a single government and, for all but one year, a single Finance Minister, so it is a recent-era pattern rather than a general law. Extending the full-Survey archive back before 2019 would let the comparison span regimes. The Surveys are also far longer than the speeches (roughly 150,000 words against 10,000), which cosine similarity on length-normalised vectors handles but which is worth stating.
:::
